...
public class VendaServico extends AbstractVendaServico {
	...
	private Venda validarVendaExiste(Long codigoVenda) {
		...
		if (venda.isEmpty()) {
			throw new RegraDeNegocioException(String.format("Venda de código %s não existe", codigoVenda));
		}
		return venda.get();
	}
	private void validarQuantidadeProdutoExiste(Produto produto, Integer qtdeVendaDto) {
		if(!(produto.getQuantidade() >= qtdeVendaDto)){
			throw new RegraDeNegocioException(String.format("A quantidade %s informada para o produto %s não está disponível em estoque", qtdeVendaDto, produto.getDescricao()));
		}
	}
	...
	private Cliente validarClienteVendaExiste(Long codigoCliente) {
		...
		if (cliente.isEmpty()) {
			throw new RegraDeNegocioException(
					String.format("O cliente de código %s informado não existe", codigoCliente));
		}
		return cliente.get();
	}
