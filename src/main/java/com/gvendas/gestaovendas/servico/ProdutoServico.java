...
public class ProdutoServico {
	...
	protected Produto validarProdutoExiste(Long codigoProduto) {
		...
		if(produto.isEmpty()) {
			throw new RegraDeNegocioException(String.format("Produto de código %s não encontrado.", codigoProduto));
		}
		return produto.get();
	}
	...
	private void validarProdutoDuplicado (Produto produto) {
		...
		if(produtoPorDescricao.isPresent() && produtoPorDescricao.get().getCodigo() != produto.getCodigo()) {
			throw new RegraDeNegocioException(String.format("O produto %s já está cadastrado", produto.getDescricao()));
		}
	}
	...
	private void validarCategoriaDoProdutoExiste(Long codigoCategoria) {
		...
		if(categoriaServico.buscarPorCodigo(codigoCategoria).isEmpty()) {
			throw new RegraDeNegocioException(String.format("A categoria de código %s informada não existe no cadastro", codigoCategoria));
		}
	}
}
